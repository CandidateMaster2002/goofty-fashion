  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => setIsPosModalOpen(true)}>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <ul className="divide-y divide-stone-200">
                {data?.rentals.slice(0, 3).map(r => (
                    <li key={r.id} className="py-3">
                        <p><strong>Rental #{r.id.split('-')[1]}</strong> status changed to <span className="font-semibold text-teal-700">{r.status}</span>.</p>
                        <p className="text-sm text-stone-500">{data?.customers.find(c => c.id === r.customer_id)?.name}</p>
                    </li>
                ))}
                {data?.customOrders.slice(0, 2).map(o => (
                    <li key={o.id} className="py-3">
                        <p><strong>Custom Order #{o.id.split('-')[1]}</strong> moved to <span className="font-semibold text-teal-700">{o.status}</span>.</p>
                        <p className="text-sm text-stone-500">{data?.customers.find(c => c.id === o.customer_id)?.name}</p>
                    </li>
                ))}
            </ul>
          </Card>
          <Card>
             ...
             <div className="space-y-3">
                <Button variant="secondary" className="w-full justify-start"><Plus size={16} className="mr-2"/> New Rental</Button>
                <Button variant="secondary" className="w-full justify-start"><Plus size={16} className="mr-2"/> New Custom Order</Button>
                <Button variant="secondary" className="w-full justify-start"><Plus size={16} className="mr-2"/> Add Inventory Item</Button>
             </div>
          </Card>
      </div>
